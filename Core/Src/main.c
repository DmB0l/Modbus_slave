    ...
    if (huart == &huart1) {
        ...
        if (func_code == 0) {
            ...
            if (func_code == 0x10 || func_code == 0x0F) {
                ...
                func_code = 0;
                modbus_process_response(device, rx_buffer, 8, response_buffer, &resp_length);
                send_flag = 1; // Устанавливаем флаг для отправки ответа
                HAL_UART_Transmit(&huart2, response_buffer, resp_length, 100); // Отладка через huart2
                HAL_UART_Receive_IT(&huart1, rx_buffer, 8); // Готовимся к следующему запросу
            }
        } else {
            func_code = 0;
            modbus_process_response(device, rx_buffer, rx_real_size, response_buffer, &resp_length);
            send_flag = 1; // Устанавливаем флаг для отправки ответа
            HAL_UART_Transmit(&huart2, response_buffer, resp_length, 100); // Отладка через huart2
            rx_real_size = 0;
            HAL_UART_Receive_IT(&huart1, rx_buffer, 8); // Готовимся к следующему запросу
